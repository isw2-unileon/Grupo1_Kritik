...
function catKey(type?: string): CatKey {
  if (type && type.toLowerCase() in CATS) return type.toLowerCase() as CatKey;
  return "game";
}
...
function Recommendations({ limit, onSeeAll }: { limit?: number; onSeeAll?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getRecommendations(preview ? limit : undefined);
  ...
  const visible = cat !== "all" && !preview
    ? products.filter((p) => p.Type?.toLowerCase() === cat)
    : products;
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((p) => (
            ...
            >
              ...
              <div className="p-4">
                <p className="line-clamp-3 text-sm text-dim">{p.Description || "Sin descripción disponible."}</p>
                <Link
                  to="/publish-review"
                  state={{ product: p }}
