...
describe("getRecommendations", () => {
  it("sends GET to /api/recommendations with auth header and returns products", async () => {
    localStorage.setItem("token", "t3");
    const fake = [{ id: 1, Name: "Game", Type: "game" }];
    mockFetch(fake);
  ...
  it("sends limit query param when provided", async () => {
    localStorage.setItem("token", "t");
    mockFetch([]);
